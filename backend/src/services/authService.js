...
exports.register = async ({ cedula, nombre, apellido, telefono, password }) => {
  if (!cedula || !nombre || !apellido || !telefono || !password) throw new Error('Todos los campos son requeridos');
  ...
  const user = new User({ cedula, nombre, apellido, telefono, password: hashed });
  return user.save();
};
